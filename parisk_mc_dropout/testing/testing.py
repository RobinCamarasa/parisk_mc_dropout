...
def main(**option_values):
    ...
    for key, value in dfs.items():
        ...
        if os.path.isfile(csv_path_):
            df = pd.read_csv(csv_path_).set_index('index')
            for column in list(output_):
                if not column in list(df):
                    df[column] = None
        else:
            df = pd.DataFrame(columns=['index'] + list(output_)).set_index('index')
        if not (EXPERIMENT_OBJECTS['experimentobject'].experiment_id in df.index):
            df.loc[EXPERIMENT_OBJECTS['experimentobject'].experiment_id] = cumulated_output_serie_
        else:
            df.update(
                pd.DataFrame(
                    [cumulated_output_serie_],
                    index=[EXPERIMENT_OBJECTS['experimentobject'].experiment_id]
                )
            )
        df.to_csv(csv_path_)
